...
SAVE_INTERVAL="${MEMPALACE_SAVE_INTERVAL:-15}"  # Save every N human messages (adjust to taste)
...
STOP_HOOK_ACTIVE=$(echo "$INPUT" | "$PYTHON_BIN" -c "import sys,json; print(json.load(sys.stdin).get('stop_hook_active', False))" 2>/dev/null)
TRANSCRIPT_PATH=$(echo "$INPUT" | "$PYTHON_BIN" -c "import sys,json; print(json.load(sys.stdin).get('transcript_path',''))" 2>/dev/null)
...
if [ -f "$TRANSCRIPT_PATH" ]; then
    EXCHANGE_COUNT=$(TRANSCRIPT_PATH="$TRANSCRIPT_PATH" "$PYTHON_BIN" - <<'PY'
import json
import os
